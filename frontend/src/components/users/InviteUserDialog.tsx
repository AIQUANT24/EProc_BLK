...
const InviteUserDialog = ({ open, onOpenChange }: InviteUserDialogProps) => {
  ...
  const handleSubmit = () => {
    ...
    addUser({ name, email, role, organization, department, status });
    toast.success(`User "${name}" invited successfully`);
    setName(""); setEmail(""); setRole("procurement"); setOrganization(""); setDepartment(""); setStatus("active");
    onOpenChange(false);
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="org" value={organization} onChange={e => setOrganization(e.target.value)} placeholder="Organization name" />
          </div>
          <div className="space-y-2">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} className="gradient-primary text-primary-foreground">Invite User</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
