...
const Compliance = () => {
  ...
  const columns: Column<ComplianceRecord>[] = [
    ...
    {
      key: "supplierName",
      label: language === "hi" ? "आपूर्तिकर्ता" : "Supplier",
      sortable: true,
    },
    {
      key: "dvaScore",
      label: "DVA",
      sortable: true,
      render: (r) => <span className="font-bold">{r.dvaScore}%</span>,
    },
    {
      key: "classification",
      label: t("dvaClassification"),
    ...
    {
      key: "confidenceScore",
      label: language === "hi" ? "विश्वसनीयता" : "Confidence",
      sortable: true,
      render: (r) => (
        <span
          className={
    ...
    },
    {
      key: "status",
      label: t("labelStatus"),
      filterOptions: ["compliant", "under-review", "non-compliant"],
      render: (r) => (
        <Badge
          variant={
            r.status === "compliant"
  ...
  const statusDist = [
    {
      name: language === "hi" ? "अनुपालक" : "Compliant",
      value: compliance.filter((c) => c.status === "compliant").length,
    },
    {
      name: language === "hi" ? "समीक्षा में" : "Under Review",
      value: compliance.filter((c) => c.status === "under-review").length,
    },
    {
      name: language === "hi" ? "गैर-अनुपालक" : "Non-Compliant",
  ...
  const avgConfidence =
    compliance.length > 0
      ? (
          (compliance.reduce((s, c) => s + c.confidenceScore, 0) /
            compliance.length) *
          100
        ).toFixed(0)
      : "0";
  const avgDva =
    compliance.length > 0
      ? (
          compliance.reduce((s, c) => s + c.dvaScore, 0) / compliance.length
        ).toFixed(1)
      : "0";
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-5 mb-8">
        <StatCard
          title={language === "hi" ? "कुल उत्पाद" : "Total Products"}
          value={compliance.length}
          icon={<ShieldCheck className="h-6 w-6" />}
        />
        <StatCard
          title={language === "hi" ? "अनुपालक" : "Compliant"}
          value={statusDist[0].value}
          icon={<CheckCircle className="h-6 w-6" />}
        />
        <StatCard
          title={language === "hi" ? "समीक्षा में" : "Under Review"}
          value={statusDist[1].value}
          icon={<AlertTriangle className="h-6 w-6" />}
        />
        <StatCard
          title={language === "hi" ? "गैर-अनुपालक" : "Non-Compliant"}
          value={statusDist[2].value}
          icon={<XCircle className="h-6 w-6" />}
        />
        <StatCard
          title={language === "hi" ? "औसत विश्वसनीयता" : "Avg Confidence"}
          value={`${avgConfidence}%`}
          subtitle={`${language === "hi" ? "औसत DVA" : "Avg DVA"}: ${avgDva}%`}
          icon={<ShieldCheck className="h-6 w-6" />}
        />
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-display text-sm">
              {language === "hi" ? "स्थिति वितरण" : "Status Distribution"}
            </CardTitle>
          </CardHeader>
          <CardContent>
